...
// Country code mapping: ISO 3166-1 alpha-2 to full country names
...
export class GLEIFService {
  ...
  /**
   * Compares region/subdivision values with fallback validation
   * For regions, we'll use a more lenient comparison since subdivisions vary greatly
   * @param region1 - First region
   * @param region2 - Second region  
   * @returns boolean
   */
  private compareRegions(region1: string, region2: string): boolean {
    ...
    // Check if one is a substring of the other (common for region variations)
    if (r1Lower.includes(r2Lower) || r2Lower.includes(r1Lower)) {
      return true
    }
    
    // For regions, we'll be more lenient and just log mismatches without failing validation
    // This is because region codes/names vary significantly between systems
    return false
  }
  ...
  async validateLocation (
    ...
    try {
      ...
      // Handle country subdivision with lenient validation
      if (country_subdivision && expected.country_subdivision && 
          !this.compareRegions(country_subdivision, expected.country_subdivision)) {
        // For regions, we log as a warning but don't fail validation due to complexity of region codes
        warnings.push(`Country subdivision mismatch: provided "${country_subdivision}", expected "${expected.country_subdivision}"`) 
        logger.warn('Region code mismatch detected but validation will continue: %s vs %s', country_subdivision, expected.country_subdivision)
      }
      ...
      if (country && !(await this.compareCountries(country, expected.country))) {
        ...
        mismatches.push(`Country mismatch: provided "${country}", expected "${expectedCountryName}"`)
      }
