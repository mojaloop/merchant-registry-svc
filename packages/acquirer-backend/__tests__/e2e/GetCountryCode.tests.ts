...
export function testGetCountryCode (app: Application): void {
  let dfspUserToken = ''
  ...
  it('should respond with 401 when Authorization header is missing', async () => {
    const res = await request(app)
      .get(`/api/v1/countries/${countryData[0].name}/code`)
  ...
  it('should respond 200 with Country Code ', async () => {
    const res = await request(app)
      .get(`/api/v1/countries/${countryData[0].name}/code`)
    ...
    expect(res.body.data).toEqual(countryData[0].code)
  })
}
