...
export function testGetDistricts (app: Application): void {
  let dfspUserToken = ''
  ...
  it('should respond with 401 when Authorization header is missing', async () => {
    const res = await request(app)
      .get(`/api/v1/countries/${countryData[0].name}/subdivisions/${countryData[0].country_subdivisions[0].name}/districts`)
  ...
  it('should respond with 200 and Country Districts List array data', async () => {
    const res = await request(app)
      .get(`/api/v1/countries/${countryData[0].name}/subdivisions/${countryData[0].country_subdivisions[0].name}/districts`)
